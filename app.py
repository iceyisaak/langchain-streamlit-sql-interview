...
if api_key:
    ...
    radio_opt=["Use SQLite3 DB", "Use POSTGRESQL DB"]
    ...
    def config_db(db_uri,host=None,user=None,pwd=None,db=None):
        if db_uri==LOCALDB:
            ...
            if not (host and user and pwd and db):
                ...
                st.stop()
            return SQLDatabase(create_engine(f"postgresql+psycopg2://{user}:{pwd}@{host}/{db}"))
    ...
    agent=create_sql_agent(
        ...
        max_iterations=50, # High limit as requested
        top_k=10
    )
